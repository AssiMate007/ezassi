      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 bg-gradient-soft scrollbar-none">
        {messages?.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-xs">Discuss the assignment, ask questions, or send an offer.</p>
          </div>
        )}
                ...
                if (m.offer_amount) {
                  return (
                    <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"} mb-2`}>
                      <div className={`max-w-[75%] rounded-3xl p-4 border-2 shadow-soft ${mine ? "bg-primary/5 border-primary/40" : "bg-card border-accent/40"}`}>
                        <p className="text-[11px] text-muted-foreground font-medium uppercase tracking-wide mb-1">{mine ? "Your offer" : "Offer received"}</p>
                  ...
                  <div key={m.id} className={`flex items-end gap-2 ${mine ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className={`flex flex-col ${mine ? "items-end" : "items-start"} max-w-[72%]`}>
                      <div className={`px-4 py-2.5 text-sm leading-relaxed shadow-xs ${
                        mine
                          ? "bg-gradient-primary text-primary-foreground rounded-[20px_20px_6px_20px]"
                          : "bg-card border border-border text-foreground rounded-[6px_20px_20px_20px]"
                      }`}>
                        {m.content}
                      </div>
      ...
      <div className="bg-card border-t border-border px-3 pt-2 pb-3">
        {showOffer && (
          <div className="mb-2 flex items-center gap-2 bg-primary/8 rounded-2xl px-3 py-2">
            ...
            <Input type="number" placeholder="Enter offer amount" value={offer} onChange={(e) => setOffer(e.target.value)}
              className="border-none bg-transparent p-0 h-auto focus-visible:ring-0 font-semibold text-primary text-base" inputMode="numeric" autoFocus />
        ...
        <form onSubmit={send} className="flex items-center gap-2">
          <button type="button" onClick={() => setShowOffer((v) => !v)}
            className={`shrink-0 h-10 w-10 rounded-2xl flex items-center justify-center transition ${showOffer ? "bg-gradient-primary text-primary-foreground shadow-soft" : "bg-muted text-muted-foreground hover:text-primary"}`}
